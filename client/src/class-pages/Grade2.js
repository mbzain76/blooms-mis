...
console.log(this.state.grade2Studs.length)
    ...
    render(){
        ...
        localStorage.clear();
        localStorage.setItem('class', 'Grade 2')
        return(
            
                          <div className="parentContainer">
        <GreyBar />
        <div className = "content">
        <ClassListMenu title="Students / Grade 2"   path={"/beginner-fee-structure"} searched = {this.state.searchText} searching={this.getSearch} filterDisplay = {this.state.filter} filter = {this.onFilterClick} searchfunc = {this.toggleSearch} searchBar = {this.state.display} toggleCheck={this.onCheckClick}/>
            
        <SideBar  emailPath={"/student-email-selected"} check={this.getChecked}/>

          <div className="main">
            <div className="componentContainer">
                <ClassListSubTab number = { this.state.grade2Studs.length}tab="Grade 2" />
               <FullNameTab />
                <DaycareList  filtered={this.state.grade2Studs} retrieveS = {this.getChecked} view = {this.state.searchText} checkVisible = {this.state.checkIcon} classLevel="Grade 2" students = {this.state.students}/>
