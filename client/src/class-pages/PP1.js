...
console.log(this.state.pp1Studs.length)
    ...
    render(){
      ...
      localStorage.setItem('class', 'PP1')
        return(
                          <div className="parentContainer">
        <GreyBar />
        <div className = "content">
        <ClassListMenu title="Students / PP1" path={"/beginner-fee-structure"} searched = {this.state.searchText} searching={this.getSearch} filterDisplay = {this.state.filter} filter = {this.onFilterClick} searchfunc = {this.toggleSearch} searchBar = {this.state.display} toggleCheck={this.onCheckClick}/>
        <SideBar  emailPath={"/student-email-selected"} check={this.getChecked}/>

          <div className="main">
            <div className="componentContainer">
                <ClassListSubTab number = { this.state.pp1Studs.length}tab="PP1" />
               <FullNameTab />
                <DaycareList  filtered={this.state.pp1Studs} retrieveS = {this.getChecked} view = {this.state.searchText} checkVisible = {this.state.checkIcon} classLevel="PP1" students = {this.state.students}/>
